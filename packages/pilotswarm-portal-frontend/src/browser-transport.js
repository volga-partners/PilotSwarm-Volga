...
export class BrowserPortalTransport {
    ...
    async saveArtifactDownload(sessionId, filename) {
        ...
        const downloadPath = `/api/sessions/${encodePathSegment(sessionId)}/artifacts/${encodePathSegment(filename)}/download`;
        const response = await fetch(
            `${this.apiBaseUrl}${downloadPath}` || downloadPath,
            { method: "GET", headers },
        );
        if (response.status === 401) {
            this.onUnauthorized();
