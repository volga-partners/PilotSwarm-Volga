...
describe("system agent cron contracts", () => {
    ...
    it("documents recurring management agents in terms of cron, not wait loops", () => {
        ...
        assertIncludes(pilotswarm, 'cron(seconds=300, reason="supervise permanent PilotSwarm system agents")', "pilotswarm should maintain its supervision loop via cron");
        assertIncludes(sweeper, "cron(seconds=300", "sweeper should establish a recurring cron schedule");
        assertIncludes(resourcemgr, "cron(seconds=900", "resource manager should establish a recurring cron schedule");
