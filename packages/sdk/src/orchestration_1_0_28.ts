 ...
 * @internal
 */
export const CURRENT_ORCHESTRATION_VERSION = "1.0.28";
 ...
 * @internal
 */
export function* durableSessionOrchestration_1_0_28(
    ctx: any,
    input: OrchestrationInput,
): Generator<any, string, any> {
    const rawTraceInfo = typeof ctx.traceInfo === "function" ? ctx.traceInfo.bind(ctx) : null;
    if (rawTraceInfo) {
        ctx.traceInfo = (message: string) => rawTraceInfo(`[v1.0.28] ${message}`);
    }
