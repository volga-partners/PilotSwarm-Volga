 ...
 * @internal
 */
export const CURRENT_ORCHESTRATION_VERSION = "1.0.27";
 ...
 * @internal
 */
export function* durableSessionOrchestration_1_0_27(
    ctx: any,
    input: OrchestrationInput,
): Generator<any, string, any> {
    const rawTraceInfo = typeof ctx.traceInfo === "function" ? ctx.traceInfo.bind(ctx) : null;
    if (rawTraceInfo) {
        ctx.traceInfo = (message: string) => rawTraceInfo(`[v1.0.27] ${message}`);
    }
        ...
        switch (result.type) {
            ...
            case "spawn_agent": {
                ...
                const subAgentPreamble =
                    ...
                    `- If a \`sql\` tool is available, do NOT use it as a scratchpad, todo list, memory store, queue, or coordination channel. ` +
                    `Use facts for durable task state and memory. Use SQL only when the user explicitly asks for database/SQL work or the task clearly requires querying or changing application data.\n` +
                    timingInstruction +
                    (canSpawnMore
