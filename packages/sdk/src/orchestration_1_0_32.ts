...
/**
 * Flat event loop durable session orchestration (v1.0.32).
 *
 ...
 * @internal
 */
export const CURRENT_ORCHESTRATION_VERSION = "1.0.32";

export function* durableSessionOrchestration_1_0_32(
    ctx: any,
    input: OrchestrationInput,
): Generator<any, string, any> {
    const rawTraceInfo = typeof ctx.traceInfo === "function" ? ctx.traceInfo.bind(ctx) : null;
    if (rawTraceInfo) {
        ctx.traceInfo = (message: string) => rawTraceInfo(`[v1.0.32] ${message}`);
    }
