...
export class PilotSwarmWorker {
  ...
  private _rawLoadedAgents: Array<{
    name: string;
    description?: string;
    prompt: string;
    tools?: string[] | null;
  ...
  private _agentPromptLookup: Record<
    string,
    {
      prompt: string;
  ...
  constructor(options: PilotSwarmWorkerOptions) {
    ...
    if (!defaultSessionStore && options.sessionStateDir) {
      const storeDir = path.join(
        path.dirname(options.sessionStateDir),
        "session-store",
      );
      defaultSessionStore = new FilesystemSessionStore(
        storeDir,
        options.sessionStateDir,
      );
    }
  ...
  async start(): Promise<void> {
    //creates the duroxide runtime and starts it, making the worker ready to accept sessions and execute activities
    ...
    if (this._catalog) {
      this.sessionManager.setDescendantSessionLookup((sessionId) =>
        this._catalog!.getDescendantSessionIds(sessionId),
      );
    }
    ...
    if (langFuseTracer)
      console.log("[PilotSwarmWorker] LangFuse tracing enabled");
      ...
      parameters: {
        ...
        properties: {
          ...
          creatableOnly: {
            type: "boolean",
            description:
              "If true, only return user-creatable (non-system) agents. Default: false",
          },
        },
      },
      handler: async (args: {
        ...
        return JSON.stringify(
          { agents: filtered, total: filtered.length },
          null,
          2,
        );
      },
  ...
  async gracefulShutdown(): Promise<void> {
    if (this.sessionStore) {
      ...
      if (ids.length > 0) {
        console.error(
          `[PilotSwarmWorker] Dehydrating ${ids.length} sessions...`,
        );
        await Promise.allSettled(
          ids.map((id) =>
            this.sessionManager.dehydrate(id, "shutdown").catch(() => {}),
          ),
        );
      }
    }
  ...
  private _loadPlugins(): void {
    ...
    if (this._loadedSkillDirs.length > 0)
      parts.push(`${this._loadedSkillDirs.length} skill dir(s)`);
    if (this._loadedAgents.length > 0)
      parts.push(
        `${this._loadedAgents.length} agent(s): ${this._loadedAgents.map((a) => a.name).join(", ")}`,
      );
    if (this._loadedSystemAgents.length > 0)
      parts.push(
        `${this._loadedSystemAgents.length} system agent(s): ${this._loadedSystemAgents.map((a) => a.name).join(", ")}`,
      );
    const mcpCount = Object.keys(this._loadedMcpServers).length;
    if (mcpCount > 0)
      parts.push(
        `${mcpCount} MCP server(s): ${Object.keys(this._loadedMcpServers).join(", ")}`,
      );
  ...
  private _loadPluginDir(
    ...
    if (fs.existsSync(policyPath)) {
      try {
        ...
        console.warn(
          `[PilotSwarmWorker] Failed to parse session-policy.json: ${err.message}`,
        );
      }
    }
  }
  ...
  private async _startSystemAgents(): Promise<void> {
    ...
    for (const agent of rootAgents) {
      ...
      try {
        ...
        console.log(
          `[PilotSwarmWorker] System agent started: ${agent.name} (${sessionId.slice(0, 8)})`,
        );
      } catch (err: any) {
        ...
        console.warn(
          `[PilotSwarmWorker] System agent ${agent.name} start failed: ${err.message}`,
        );
      }
    }
  }
  ...
  private async _createProvider(): Promise<any> {
    ...
    if (store.startsWith("postgres://") || store.startsWith("postgresql://")) {
      return PostgresProvider.connectWithSchema(
        store,
