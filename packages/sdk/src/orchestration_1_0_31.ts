...
export function* durableSessionOrchestration_1_0_31(
    ...
    if (rawTraceInfo) {
        ctx.traceInfo = (message: string) => rawTraceInfo(`[v1.0.31] ${message}`);
    }
    ...
    function* queueFollowupAndMaybeContinue(nextPrompt: string): Generator<any, boolean, any> {
        const extracted = extractPromptSystemContext(nextPrompt);
        pendingPrompt = mergePrompt(pendingPrompt, extracted.prompt);
        pendingSystemPrompt = mergePrompt(pendingSystemPrompt, extracted.systemPrompt);
    ...
    function* versionedContinueAsNew(input: OrchestrationInput): Generator<any, void, any> {
        ...
        yield ctx.continueAsNewVersioned(input, DURABLE_SESSION_LATEST_VERSION);
    }
    ...
    while (true) {
        ...
        // ① GET NEXT PROMPT
            ...
            // The actual prompt injection now happens in SessionManager via the
            // Copilot SDK's tool_instructions section transform. Keep this load
            // activity in the orchestration path for observability and, more
            // importantly, to preserve the existing durable yield sequence.
